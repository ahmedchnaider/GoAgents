...
const Signup: React.FC = () => {
  ...
  // Update the plans array to only include Silver, Gold, and Diamond (no Free card)
  const plans = [
    { id: 'silver', title: 'Silver', description: '1000 Credits · 2 Agents', price: '$10' },
    { id: 'gold', title: 'Gold', description: '5000 Credits · 5 Agents', price: '$25' },
    { id: 'diamond', title: 'Diamond', description: 'Unlimited Credits · 20 Agents', price: '$99' }
  ];
  ...
  const completeSignup = async (): Promise<void> => {
    try {
      ...
      const signupData = {
        ...
        plan: selectedPlan || 'free' // Use selected plan or 'free' if none selected
      };
      ...
      <ContentWrapper>
        <FormSection>
          <HeaderSection>
            {!isMobile && (
              <>
                <HeaderTitle style={{ paddingTop: '18px' }}>Create your account</HeaderTitle>
                <HeaderSubtitle style={{ marginBottom: '18px' }}>Get started with your free account</HeaderSubtitle>
              </>
            )}
            {isMobile && (
              <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'center', gap: '16px' }}>
                {plans.map(plan => (
                  <PlanCard
                    ...
                    style={{
                      background: selectedPlan === plan.id ? 'white' : 'rgba(255,255,255,0.12)',
                      color: selectedPlan === plan.id ? '#4754F0' : 'white',
                      borderColor: selectedPlan === plan.id ? '#fff' : 'rgba(255,255,255,0.3)',
                      minWidth: 90,
                      textAlign: 'center',
                      fontWeight: 600,
                      fontSize: 16,
            ...
            <form onSubmit={handleSubmit}>
              ...
              <Button type="submit" disabled={loading}>
                {loading
                  ? 'Creating Account...'
                  : isMobile
                    ? `Create ${selectedPlan ? plans.find(p => p.id === selectedPlan)?.title : 'Free'} Account`
                    : 'Create Account'}
              </Button>
